...
class TestPaperCache(unittest.TestCase):
    ...
    def test_cleanup_stale_cache_entries_removes_missing_pdf_and_artifacts(self):
        with tempfile.TemporaryDirectory() as tmp:
            stale_pdf = Path(tmp) / "missing.pdf"
            cache.record_download(
                pdf_path=str(stale_pdf),
                paper_key_hint="stale-paper",
                source="local",
                title="Stale Paper",
                cache_dir=tmp,
            )
            stale_paths = cache.get_cached_paths("stale-paper", cache_dir=tmp)
            cache.write_json(stale_paths["content_list"], [{"id": "s1", "text": "stale searchable text"}])
            cache.index_parsed_paper("stale-paper", cache_dir=tmp)
            Path(stale_paths["content_list"]).unlink()
            self.assertTrue(Path(stale_paths["paper_dir"]).exists())
            self.assertEqual(len(cache.search_parsed_index("stale", cache_dir=tmp)), 1)

            kept_pdf = Path(tmp) / "kept.pdf"
            kept_pdf.write_bytes(b"%PDF-1.4\n%%EOF")
            cache.record_download(
                pdf_path=str(kept_pdf),
                paper_key_hint="kept-paper",
                source="local",
                title="Kept Paper",
                cache_dir=tmp,
            )

            dry_run = cache.cleanup_stale_cache_entries(cache_dir=tmp)
            self.assertEqual(dry_run["status"], "dry_run")
            self.assertEqual(dry_run["stale_total"], 1)
            self.assertEqual(dry_run["stale"][0]["paper_key"], "stale-paper")
            self.assertTrue(Path(stale_paths["paper_dir"]).exists())

            applied = cache.cleanup_stale_cache_entries(cache_dir=tmp, dry_run=False)
            self.assertEqual(applied["status"], "ok")
            self.assertEqual(applied["stale_total"], 1)
            self.assertFalse(Path(stale_paths["paper_dir"]).exists())
            self.assertTrue(Path(cache.get_cached_paths("kept-paper", cache_dir=tmp)["paper_dir"]).exists())
            self.assertEqual(cache.search_parsed_index("stale", cache_dir=tmp), [])
